...
export const ActiveFlowsRow = ({ item }: { item: ReturnType<typeof useActiveFlowList>['list']['0'] }) => {
  ...
  const paused =
    item.raw.health.unblocked === false || item.raw.health.unpaused === false;
  ...
  const errorMsg =
    item.raw.error && item.raw.error.info && item.raw.error.info.msg;

  const isRegularPaused = item.raw.blocked.some((el) => el.blocked === true);

  const isSelfRegularPaused = item.raw.blocked.some(
    (el) => el.blocked === true && el.address === vaultAddress
  );

  const isVirtualPausedExist = item.raw.pause && Boolean(item.raw.pause.length);

  const isVirtualPaused =
    isVirtualPausedExist && item.raw.pause.some((el) => el.blocked === true);

  const isSelfVirtualPaused =
  ...
  const pauseState = {
    regular: getPauseState(
      disabledPause,
      isSelfRegularPaused,
      isRegularPaused
    ),
    virtual: getPauseState(
      disabledPause,
      isSelfVirtualPaused,
      isVirtualPaused
    ),
  ...
  const neededApprovals = (needed?.approvals || []).filter(
    (el) => !el.approved
  );

  const neededVaultApprovals = neededApprovals.filter(
    (el) => el.address.toLowerCase() === vaultAddress.toLowerCase()
  );
  const neededWalletApprovals = neededApprovals.filter(
    (el) => el.address.toLowerCase() === walletAddress.toLowerCase()
  );
  const neededExternalApprovals =
    neededApprovals.length >
    neededVaultApprovals.length + neededWalletApprovals.length;
  ...
  const renderStatus = () => {
    ...
    const PauseButton = <FCTPauseButton
      id={id}

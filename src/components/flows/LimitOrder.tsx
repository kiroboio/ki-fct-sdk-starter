...
const CurrencyOption: React.FC<CurrencyOptionProps> = ({ token, isSelected, onSelect }) => (
  <LinkBox py={1} _hover={{ background: 'gray.100' }}>
    <LinkOverlay
      href="#"
      onClick={() => {
        onSelect(token)
...
export const LimitOrder = () => {
  ...
  const getUniswapParams = async ({
    amountIn,
    amountOut,
    isExactIn,
    type,
  }: {
    isExactIn: 'true' | 'false'
    type: 'swap' | 'limit_price'
    amountIn?: string
    amountOut?: string
  }) => {
    if (!amountIn && !amountOut) return
    const simulateSwap = new FCT_UNISWAP.actions.SwapNoSlippageProtection({
      chainId,
      provider: service.providers.smartRpc(),
      initParams: {
        addressIn: fromToken?.address,
        addressOut: toToken?.address,
